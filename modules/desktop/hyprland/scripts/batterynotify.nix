...
pkgs.writeShellScriptBin "batterynotify" ''
  ...
  main() {
    if is_laptop; then
      ...
      notify="''${notify:-1140}"
      interval="''${interval:-5}"
      execute="''${execute:-${pkgs.systemd}/bin/systemctl suspend}"
  ...
  EOF
      if $verbose; then
        ...
        current_pid=$$
        pids=$(${pkgs.procps}/bin/pgrep -f "batterynotify" | grep -v "^$current_pid$")
        for pid in $pids ; do
          if [ $pid -ne $current_pid ] ; then
            kill -STOP $pid
            ${pkgs.libnotify}/bin/notify-send -a "Battery Notify" -t 2000 -r 9889 -u "CRITICAL" "Debugging STARTED, Pausing Regular Process"
          fi
        done
        trap resume_processes SIGINT
      fi
      ...
      ${pkgs.dbus}/bin/dbus-monitor --system "type='signal',interface='org.freedesktop.DBus.Properties',path='$(${pkgs.upower}/bin/upower -e | grep battery)'" 2> /dev/null | while read -r battery_status_change; do fn_status_change ; done
    fi
  }
  main
''
